...
import { SpringenPaginationProps } from './types';
...
const Pagination: React.FC<SpringenPaginationProps> = ({
  value,
  totalSize,
  pageSize = 10,
  onChange,
  ...
  const handleItemMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (!hoverFillVisibleRef.current) {
      ...
      hoverFillBgRef.current.style.transformOrigin = `${clientX - x}px ${clientY - y}px`;
    } else {
      hoverFillRef.current.style.transitionDuration = '0.25s';
    }
  };
    ...
    setTimeout(() => {
      ...
      if (!currentEl) {
        ...
        if (
          ...
          (value === pageNumber &&
            hoverFillRef.current.offsetLeft + hoverFillRef.current.offsetWidth >= containerRef.current.offsetWidth)
        ) {
          hoverFillBgRef.current.style.background = 'rgba(33, 34, 38, 0)';
        }
      }
  ...
  const endPages = range(Math.max(pageNumber - boundaryCount + 1, boundaryCount + 1), pageNumber);
